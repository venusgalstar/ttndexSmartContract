...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when Bridgeswap mining ends.
    uint256 public bonusEndBlock;

    // The block number when Bridgeswap mining starts.
    ...
    // Bridgeswap tokens created per block.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 bridgeswapReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(bridgeswapReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 bridgeswapReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(bridgeswapReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
